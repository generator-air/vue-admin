<template>
  <div id="app">
    ...
    <div class="login" v-if="user && !user.unAuth && !user.unLogin">
      ...
      <div class="l-right">
        <v-head class="l-header" title="后台管理系统" />
        <div class="l-body">
          <div class="body-content">
            <div class="l-bread">
              <el-breadcrumb separator="/" v-if="$route.meta.breadcrumb">
                <el-breadcrumb-item
              ...
              <h3>{{ $route.name }}</h3>
              <router-view class="l-route"></router-view>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- 已登录、无权限用户 -->
    <div class="un-auth" v-else-if="user">
      <router-view />
    </div>
  </div>
</template>
