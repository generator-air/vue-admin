...
function createFile(projectRoot, rootPath, folderName, overwrite, indexName) {
  const arr = [];
  let routerList = '';
  let imports = '';
  // 已存在路由文件的内容字符串
  ...
  if (!overwrite) {
    fileStr = $fse
      .readFileSync(`${projectRoot}/src/router/${folderName}.js`)
      .toString();
  }
  // 拿到当前文件夹下，所有文件的路径（存于arr）
  getPagePaths(projectRoot, rootPath, folderName, arr);
  arr.forEach((path) => {
    ...
    // 构造路由描述字符串（带着缩进格式。使用 2 space 缩进）
    const routerStr = `  {\n    path: '${routerPath}',\n    component: ${name},\n  },\n`;
    if (!overwrite) {
  ...
  if (!overwrite) {
    // 执行文件创建与输出
    outputFile(projectRoot, folderName, fileStr);
  } else {
    // 路由文件内容生成
    const contentStr = `${imports}\nconst routerList = [\n${routerList}];\n\nexport default routerList;\n`;
    // 执行文件创建与输出
    outputFile(projectRoot, folderName, contentStr);
  }
}

// 获取所有的 vue page 文件的路径
function getPagePaths(projectRoot, rootPath, folderName, arr) {
  const data = $fse.readdirSync(
    `${projectRoot}/${rootPath}/${folderName}`,
    'binary'
  );
  data &&
    data.forEach((item) => {
      // 如果存在二级、三级...目录
      if (isFolder(`${projectRoot}/${rootPath}/${folderName}/${item}`)) {
        getPagePaths(projectRoot, `${rootPath}/${folderName}`, item, arr);
      } else {
...
// 文件生成，输出
function outputFile(projectRoot, fileName, content) {
  $fse.outputFile(
    `${projectRoot}/src/router/${fileName}.js`,
    content,
    function (err) {
      if (err) {
...
$inquirer
  ...
  .then((answer) => {
    if (answer.confirm) {
      $fse.readdir(`${projectRoot}/src/${answer.pagePath}`, 'binary', function (
        err,
        data
      ) {
        data &&
          data.forEach((pageItem) => {
            // 遍历pages下的每一个文件夹
            if (isFolder(`${projectRoot}/src/${answer.pagePath}/${pageItem}`)) {
              // 判断当前文件夹对应路由文件是否存在
              $fse.pathExists(
                `${projectRoot}/src/router/${pageItem}.js`,
                (err, exists) => {
                  // 如果路由文件已存在
                  if (exists) {
                    createFile(
